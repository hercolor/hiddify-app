                      ...
                      _In_ wchar_t *command_line, _In_ int show_command)
{
  ...
  if (SendAppLinkToInstance(L"BflyVPN"))
  {
    return EXIT_SUCCESS;
  }

  HANDLE hMutexInstance = CreateMutex(NULL, TRUE, L"BflyVPNMutex");
  HWND handle = FindWindowW(NULL, L"BflyVPN");
  ...
  if (GetLastError() == ERROR_ALREADY_EXISTS)
  {
    ...
    if (window.SendAppLinkToInstance(L"BflyVPN"))
    {
      return false;
    }
  ...
  if (!window.Create(L"BflyVPN", origin, size))
  {
    return EXIT_FAILURE;
  }
