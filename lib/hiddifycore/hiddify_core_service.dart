  ...
  TaskEither<String, Unit> stop() {
    return TaskEither(() async {
      ...
      try {
        ...
        final benignStopError =
            e.code == StatusCode.unavailable ||
            message.contains("HTTP/2") ||
            message.toLowerCase().contains("not started") ||
            message.toLowerCase().contains("already stopped") ||
  ...
  Future<void> closeFront() async {
    ...
    if (!core.isSingleChannel()) {
      ...
      try {
        ...
        // Best-effort close for the insecure front channel.
      }
      try {
        await core.fgClient.close(CloseRequest(mode: SetupMode.GRPC_NORMAL));
      } catch (_) {
        // Best-effort close for the secure front channel.
      }
    }
  }
}
