...
| Phase 1b | this commit | Route boundary test + blocking audit gate | Constraint/Directive/Tested/Not-tested recorded in commit | dart format route test; flutter test safe_node_display_name_test; flutter test membership_route_boundary_test; dart analyze route test; flutter analyze; phase-1 forbidden audit + classifier; task-owned diff guard | pass-with-existing-dirty-tree-noted | Adds explicit `/settings` Membership/Login boundary test required by renewed plan; broad native/core dirty tree remains pre-existing and unstaged. |
...
| Phase 1 | lib/core/router/bottom_sheets/widgets/auto_apps_selection_modal.dart | settings.routing.perAppProxy.autoSelection | Existing internal modal | diagnostics | Pre-existing technical/per-app selection bottom sheet outside normal Home/Nodes/Membership UI; not introduced by rollout. | none |
| Phase 1 | lib/core/router/dialog/widgets/setting_text_dialog.dart | settings.routing.routeRule | Existing internal dialog | diagnostics | Pre-existing technical settings dialog outside normal UI; not introduced by rollout. | none |
| Phase 1 | lib/core/router/dialog/widgets/action_at_closing_dialog.dart | settings.general.actionAtClosing | Existing internal dialog | diagnostics | Pre-existing closing behavior dialog outside normal Home/Nodes/Membership UI; not introduced by rollout. | none |
